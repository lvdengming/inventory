  ...
  return (
    <div className="px-4">
        ...
        <div className="rounded-2xl bg-[var(--color-card)] overflow-hidden shadow-sm">
          {(['light', 'dark'] as const).map((mode, idx) => (
            ...
            >
              ...
              {theme === mode && <span className="text-[var(--color-primary)]">✓</span>}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="rounded-2xl bg-[var(--color-card)] overflow-hidden shadow-sm">
          {items.length === 0 ? (
            <div className="px-4 py-3 text-[var(--color-text-secondary)] text-sm">暂无物品</div>
          ) : (
            items.map((item, idx) => (
              <div
                key={item.id}
              ...
              >
                ...
                <button onClick={() => removeItem(item.id)} className="text-red-500 text-[15px]">
                  删除
                </button>
              </div>
            ))
...
function AddItemModal({ onClose, onAdd }: { onClose: () => void; onAdd: (item: Item) => void }) {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end z-50">
      <div className="w-full bg-[var(--color-bg)] rounded-t-3xl p-5 pb-[env(safe-area-inset-bottom)] max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-5">
          <button onClick={onClose} className="text-[var(--color-primary)] text-[17px]">
            取消
          </button>
          <h3 className="text-[17px] font-semibold">添加物品</h3>
          <button onClick={handleSubmit} className="text-[var(--color-primary)] text-[17px] font-semibold">
            完成
          </button>
        </div>
        <div className="space-y-3">
          <input placeholder="物品名称" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputClass} />
          ...
          <input type="number" placeholder="购买金额" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className={inputClass} />
          <textarea placeholder="备注" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={inputClass} rows={2} />
          ...
          {image && <img src={image} className="w-20 h-20 object-cover rounded-xl mx-auto" />}
        </div>
      </div>
    </div>
  );
